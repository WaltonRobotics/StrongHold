...
/**
 * Library ported from Adafruit Ardunio library. See below for original license info. IMU Library not included, routines
 * return arrays of double. Code does not use the sensor API from Adafruit -- call getVector/getQuaternion to get data.
 * Otherwise use as directed.
 *
 ...
 * orientation sensor
 *
 * Designed specifically to work with the Adafruit BNO055 Breakout. Pick one up today in the adafruit shop! ------>
 * http://www.adafruit.com/products
 *
 * These sensors use I2C to communicate, 2 pins are required to interface.
 *
 * Adafruit invests time and resources providing this open source code, please support Adafruit andopen-source hardware
...
public class BNO055 {
	 ...
	 *
	 * @param mode Desired operating mode after initialization
	 * @return True if initialization succeeded
	 */
	 ...
	 *
	 * @param mode OperationMode enumeration value selecting operating mode
	 */
	 ...
	 *
	 * @param usextal Use crystal when true
	 */
	 ...
	 *
	 * @param vectorType VectorType enumeration value to select which vector to return
	 ...
	 *
	 * @param offsets Offsets data from call to getSensorOffsets
	 */
	...
	private enum Register {
		...
		/**
		 * Create a scalar register which represents an arbitrary number of bytes (assumed little-endian)
		 *
		 * @param address The address of the register
		 * @param bytes The number of bytes which store the value at the register
		 */
		...
		/**
		 * Create a scalar register which represents and arbitrary number of bytes (assumed little-endian) which can be
		 * signed
		 *
		 * @param address The address of the register
		 * @param bytes The number of bytes which store the value at the register
		 * @param signed The value stored at the register is a signed value
		 */
		...
		/**
		 * Create a vector register with an arbitrary number of bytes (assumed little-endian)
		 *
		 * @param address The address of the register
		 * @param bytes The number of bytes which store the value at the register
		 ...
		 *
		 * @param bno055 I2C object connected to the BNO055 chip
		 ...
		 *
		 * @param bno055 I2C object connected to the BNO055 chip
		 * @param scaleFactor Scaling factor to apply to values
		 * @return Vector of values
		 */
		 ...
		 *
		 * Number of bytes written defined by register. Number of elements in vector must match.
		 *
		 * @param bno055 I2C object connected to the BNO055 chip
	 ...
	 *
	 * System Status (see section 4.3.58) 0 = Idle 1 = System Error 2 = Initializing Peripherals 3 = System
	 * Iniitalization 4 = Executing Self-Test 5 = Sensor fusion algorithm running 6 = System running without fusion
	 * algorithms
	 *
	 * Self Test Results (see section ) 1 = test passed, 0 = test failed Bit 0 = Accelerometer self test Bit 1 =
	 * Magnetometer self test Bit 2 = Gyroscope self test Bit 3 = MCU self test 0x0F = all good!
	 *
	 * System Error (see section 4.3.59) 0 = No error 1 = Peripheral initialization error 2 = System initialization
	 * error 3 = Self test result failed 4 = Register map value out of range 5 = Register map address out of range 6 =
	 * Register map write error 7 = BNO low power mode not available for selected operation mode 8 = Accelerometer power
	 * mode not available 9 = Fusion algorithm configuration error A = Sensor configuration error
	 */
